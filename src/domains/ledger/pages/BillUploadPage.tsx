...
import { classifyAccounting, classifyTextQuick, classifyText } from '../api/ledger-api';
...
const PageBillUpload: React.FC = () => {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div>
        <label className="inline-flex items-center gap-2 px-3 py-2 rounded border border-border-light text-sm text-text-secondary hover:text-primary hover:border-primary cursor-pointer">
          <i className="fas fa-camera"></i>
          <span>拍照上传账单</span>
          <input type="file" accept="image/*" capture="environment" className="hidden" onChange={e => onFiles(e.target.files)} />
        </label>
      </div>
      <div className="rounded-lg border border-primary/30 bg-primary/5 p-3 text-sm text-text-secondary space-y-1">
        <p><i className="fas fa-camera text-primary mr-2"></i>支持拍照上传账单，识别更快。</p>
        <p><i className="fas fa-cloud-sun text-warning mr-2"></i>今天天气变化较快，记得多穿一件外套哦。</p>
...
const TextQuickClassify: React.FC<{ onParsed: (res:any)=>void; onGoto:(res:any)=>void }>= ({ onParsed, onGoto }) => {
  ...
  const startVoice = () => {
    ...
    recognition.onerror = () => setError('语音识别失败');
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-start gap-2">
        ...
        <button type="button" onClick={startVoice} className={`w-10 h-10 rounded border border-border-light ${listening ? 'bg-primary text-white' : 'text-text-secondary hover:text-primary hover:border-primary'}`} title="语音输入">
