...
const TopBarActions: React.FC = () => {
  ...
  const onSearch = () => {
    ...
    else if (q.includes('分析')) window.location.assign('/analysis');
    else if (q.includes('规划')) window.location.assign('/planning');
    else if (q.includes('产品') || q.includes('理财')) window.location.assign('/products');
      ...
      {showNotify && (
        <div ref={notifyRef} className="absolute top-full right-6 mt-2 bg-white border border-border-light rounded-lg shadow-lg p-0 w-80 z-50">
          ...
          <div className="max-h-64 overflow-auto divide-y">
            <div className="px-4 py-3 text-sm hover:bg-gray-50 cursor-pointer">欢迎使用算小智，祝您财务顺利！</div>
